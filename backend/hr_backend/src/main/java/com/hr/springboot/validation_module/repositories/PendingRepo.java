...
public interface PendingRepo extends JpaRepository<Pending_request, Integer>{
	@Query(value = "select * from pending_request where approved_by_l3 = 0 AND approved_by_l2 = 0", nativeQuery = true)
	List<Pending_request> findL3();

	@Query(value = "select * from pending_request where approved_by_l3 = 0 AND approved_by_l2 = 0", nativeQuery = true)
	List<Pending_request> findL2();

	@Query(value = "select * from pending_request where (approved_by_l3 = 1 OR approved_by_l2 = 1) AND approved_by_l1 = 0", nativeQuery = true)
